...
class BaicellsSmsOptionsFlow(config_entries.OptionsFlow):
    """Handle an options flow for Baicells SMS.

    The active config entry is exposed automatically by Home Assistant via
    the inherited ``config_entry`` property; it must not be stored manually
    in ``__init__``, since that pattern is deprecated on current Home
    Assistant releases and can raise "Invalid handler specified" errors.
    """
